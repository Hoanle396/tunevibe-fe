...
const IndexTrends = ({ trendsInfo }: { trendsInfo: Music[] }) => {
  const [music, setMusic, setPlaylist, playlistId] = useAppStore((state) => [
    state.currentMusic,
    state.setMusic,
    state.setPlaylist,
    state.playListId,
  ...
  return (
    <Swiper
      className={`${styles.slider} trend-slider rounded-xl`}
      slidesPerView={1}
      centeredSlides={true}
      autoplay={{
        delay: 8000,
